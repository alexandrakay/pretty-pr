...
var require_event_target_shim = __commonJS({
  "node_modules/event-target-shim/dist/event-target-shim.js"(exports2, module2) {
    ...
    function pd(event2) {
      const retv = privateData.get(event2);
      console.assert(
        retv != null,
    ...
    function Event(eventTarget, event2) {
      privateData.set(this, {
        ...
        timeStamp: event2.timeStamp || Date.now()
      });
      Object.defineProperty(this, "isTrusted", { value: false, enumerable: true });
      const keys = Object.keys(event2);
      for (let i2 = 0; i2 < keys.length; ++i2) {
        const key = keys[i2];
        if (!(key in this)) {
    ...
    function defineCallDescriptor(key) {
      return {
        value() {
          const event2 = pd(this).event;
          return event2[key].apply(event2, arguments);
        },
        configurable: true,
        enumerable: true
      };
    ...
    function defineWrapper(BaseEvent, proto) {
      ...
      function CustomEvent(eventTarget, event2) {
        BaseEvent.call(this, eventTarget, event2);
      }
    ...
    function wrapEvent(eventTarget, event2) {
      const Wrapper = getWrapper(Object.getPrototypeOf(event2));
      return new Wrapper(eventTarget, event2);
    }
    function isStopped(event2) {
      return pd(event2).immediateStopped;
    }
    function setEventPhase(event2, eventPhase) {
      pd(event2).eventPhase = eventPhase;
    }
    function setCurrentTarget(event2, currentTarget) {
      pd(event2).currentTarget = currentTarget;
    }
    function setPassiveListener(event2, passiveListener) {
      pd(event2).passiveListener = passiveListener;
    }
    ...
    EventTarget.prototype = {
      ...
      dispatchEvent(event2) {
        if (event2 == null || typeof event2.type !== "string") {
          throw new TypeError('"event.type" should be a string.');
        }
        const listeners = getListeners(this);
        const eventName = event2.type;
        let node = listeners.get(eventName);
        if (node == null) {
          return true;
        }
        const wrappedEvent = wrapEvent(this, event2);
        let prev = null;
        while (node != null) {
          if (node.once) {
            if (prev !== null) {
...
var BetaMessageStream = class _BetaMessageStream {
  ...
  async _createMessage(messages, params, options) {
    ...
    for await (const event2 of stream) {
      __classPrivateFieldGet8(this, _BetaMessageStream_instances, "m", _BetaMessageStream_addStreamEvent).call(this, event2);
    }
   ...
   */
  on(event2, listener) {
    const listeners = __classPrivateFieldGet8(this, _BetaMessageStream_listeners, "f")[event2] || (__classPrivateFieldGet8(this, _BetaMessageStream_listeners, "f")[event2] = []);
   ...
   */
  off(event2, listener) {
    const listeners = __classPrivateFieldGet8(this, _BetaMessageStream_listeners, "f")[event2];
    if (!listeners)
      return this;
  ...
  _emit(event2, ...args) {
    ...
    const listeners = __classPrivateFieldGet8(this, _BetaMessageStream_listeners, "f")[event2];
    if (listeners) {
      __classPrivateFieldGet8(this, _BetaMessageStream_listeners, "f")[event2] = listeners.filter((l2) => !l2.once);
  ...
  [(_BetaMessageStream_currentMessageSnapshot = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_connectedPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_resolveConnectedPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_rejectConnectedPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_endPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_resolveEndPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_rejectEndPromise = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_listeners = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_ended = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_errored = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_aborted = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_catchingPromiseCreated = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_response = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_request_id = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_handleError = /* @__PURE__ */ new WeakMap(), _BetaMessageStream_instances = /* @__PURE__ */ new WeakSet(), _BetaMessageStream_getFinalMessage = function _BetaMessageStream_getFinalMessage2() {
  ...
  }, _BetaMessageStream_addStreamEvent = function _BetaMessageStream_addStreamEvent2(event2) {
    if (this.ended)
      return;
    const messageSnapshot = __classPrivateFieldGet8(this, _BetaMessageStream_instances, "m", _BetaMessageStream_accumulateMessage).call(this, event2);
    this._emit("streamEvent", event2, messageSnapshot);
    switch (event2.type) {
      case "content_block_delta": {
        const content = messageSnapshot.content.at(-1);
        switch (event2.delta.type) {
          case "text_delta": {
            if (content.type === "text") {
              this._emit("text", event2.delta.text, content.text || "");
            }
            break;
          }
          case "citations_delta": {
            if (content.type === "text") {
              this._emit("citation", event2.delta.citation, content.citations ?? []);
            }
            break;
          }
          case "input_json_delta": {
            if (content.type === "tool_use" && content.input) {
              this._emit("inputJson", event2.delta.partial_json, content.input);
            }
            break;
          }
          case "thinking_delta": {
            if (content.type === "thinking") {
              this._emit("thinking", event2.delta.thinking, content.thinking);
            }
            break;
          }
    ...
    __classPrivateFieldSet7(this, _BetaMessageStream_currentMessageSnapshot, void 0, "f");
    return snapshot;
  }, _BetaMessageStream_accumulateMessage = function _BetaMessageStream_accumulateMessage2(event2) {
    let snapshot = __classPrivateFieldGet8(this, _BetaMessageStream_currentMessageSnapshot, "f");
    if (event2.type === "message_start") {
      if (snapshot) {
        throw new AnthropicError(`Unexpected event order, got ${event2.type} before receiving "message_stop"`);
      }
      return event2.message;
    }
    if (!snapshot) {
      throw new AnthropicError(`Unexpected event order, got ${event2.type} before "message_start"`);
    }
    switch (event2.type) {
      case "message_stop":
        return snapshot;
      case "message_delta":
        snapshot.stop_reason = event2.delta.stop_reason;
        snapshot.stop_sequence = event2.delta.stop_sequence;
        snapshot.usage.output_tokens = event2.usage.output_tokens;
        return snapshot;
      case "content_block_start":
        snapshot.content.push(event2.content_block);
        return snapshot;
      case "content_block_delta": {
        const snapshotContent = snapshot.content.at(event2.index);
        switch (event2.delta.type) {
          case "text_delta": {
            if (snapshotContent?.type === "text") {
              snapshotContent.text += event2.delta.text;
            }
            break;
          }
          case "citations_delta": {
            if (snapshotContent?.type === "text") {
              ...
              snapshotContent.citations.push(event2.delta.citation);
            }
            break;
          }
          case "input_json_delta": {
            if (snapshotContent?.type === "tool_use") {
              ...
              jsonBuf += event2.delta.partial_json;
          ...
          case "thinking_delta": {
            if (snapshotContent?.type === "thinking") {
              snapshotContent.thinking += event2.delta.thinking;
            }
            break;
          }
          case "signature_delta": {
            if (snapshotContent?.type === "thinking") {
              snapshotContent.signature = event2.delta.signature;
            }
            break;
          }
          default:
...
var MessageStream = class _MessageStream {
  ...
  async _createMessage(messages, params, options) {
    ...
    this._connected(response);
    for await (const event2 of stream) {
      __classPrivateFieldGet9(this, _MessageStream_instances, "m", _MessageStream_addStreamEvent).call(this, event2);
    }
    if (stream.controller.signal?.aborted) {
      throw new APIUserAbortError();
    }
   ...
   */
  on(event2, listener) {
    const listeners = __classPrivateFieldGet9(this, _MessageStream_listeners, "f")[event2] || (__classPrivateFieldGet9(this, _MessageStream_listeners, "f")[event2] = []);
    listeners.push({ listener });
    return this;
  }
  /**
   ...
   */
  off(event2, listener) {
    const listeners = __classPrivateFieldGet9(this, _MessageStream_listeners, "f")[event2];
    if (!listeners)
      return this;
    const index = listeners.findIndex((l2) => l2.listener === listener);
    if (index >= 0)
  ...
  _emit(event2, ...args) {
    ...
    const listeners = __classPrivateFieldGet9(this, _MessageStream_listeners, "f")[event2];
    if (listeners) {
      __classPrivateFieldGet9(this, _MessageStream_listeners, "f")[event2] = listeners.filter((l2) => !l2.once);
      listeners.forEach(({ listener }) => listener(...args));
    }
    if (event2 === "abort") {
      const error = args[0];
  ...
  [(_MessageStream_currentMessageSnapshot = /* @__PURE__ */ new WeakMap(), _MessageStream_connectedPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_resolveConnectedPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_rejectConnectedPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_endPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_resolveEndPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_rejectEndPromise = /* @__PURE__ */ new WeakMap(), _MessageStream_listeners = /* @__PURE__ */ new WeakMap(), _MessageStream_ended = /* @__PURE__ */ new WeakMap(), _MessageStream_errored = /* @__PURE__ */ new WeakMap(), _MessageStream_aborted = /* @__PURE__ */ new WeakMap(), _MessageStream_catchingPromiseCreated = /* @__PURE__ */ new WeakMap(), _MessageStream_response = /* @__PURE__ */ new WeakMap(), _MessageStream_request_id = /* @__PURE__ */ new WeakMap(), _MessageStream_handleError = /* @__PURE__ */ new WeakMap(), _MessageStream_instances = /* @__PURE__ */ new WeakSet(), _MessageStream_getFinalMessage = function _MessageStream_getFinalMessage2() {
  ...
  }, _MessageStream_addStreamEvent = function _MessageStream_addStreamEvent2(event2) {
    if (this.ended)
      return;
    const messageSnapshot = __classPrivateFieldGet9(this, _MessageStream_instances, "m", _MessageStream_accumulateMessage).call(this, event2);
    this._emit("streamEvent", event2, messageSnapshot);
    switch (event2.type) {
      case "content_block_delta": {
        const content = messageSnapshot.content.at(-1);
    ...
    __classPrivateFieldSet8(this, _MessageStream_currentMessageSnapshot, void 0, "f");
    return snapshot;
  }, _MessageStream_accumulateMessage = function _MessageStream_accumulateMessage2(event2) {
...
var ANTHROPIC_API_KEY = INPUT_ANTHROPIC_API_KEY || RAW_ANTHROPIC_API_KEY;
var GITHUB_TOKEN = INPUT_GITHUB_TOKEN || RAW_GITHUB_TOKEN;
var MODE = INPUT_MODE || "diff";
var POST_IF_EMPTY = INPUT_POST_IF_EMPTY ?? "true";
var SLACK_WEBHOOK_URL = INPUT_SLACK_WEBHOOK_URL || "";
var event = JSON.parse((0, import_fs2.readFileSync)(process.env.GITHUB_EVENT_PATH, "utf-8"));
var PR_NUMBER = String(event.pull_request?.number || "");
var PR_BODY = event.pull_request?.body || "";
var PR_TITLE = event.pull_request?.title || "";
var PR_URL = event.pull_request?.html_url || "";
var PR_AUTHOR = event.pull_request?.user?.login || "";
...
function run(cmd) {
  try {
    return (0, import_child_process.execSync)(cmd, { encoding: "utf-8", stdio: ["pipe", "pipe", "pipe"] }).trim();
  } catch {
    return "";
  }
}
